...
export async function GET(req: NextRequest) {
    ...
    try {
        const url = `${baseUrl}/api/v5/subscriptions?${searchParams.toString()}`;
        ...
        if (!response.ok) {
            ...
            return NextResponse.json({ error: errorText, details: `Upstream ${response.status}` }, { status: response.status });
        }
